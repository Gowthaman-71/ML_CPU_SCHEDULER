    ...
    def run_init():
        ...
        for i in range(max_retries):
            try:
                ...
                print(f"❌ [INIT] Initialization attempt {i+1} failed: {e}", flush=True)
                traceback.print_exc()
...
def get_processes():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        query = "SELECT * FROM process_data WHERE 1=1"
        params = []
        ...
        query += " ORDER BY arrival_time DESC LIMIT %s"
...
# ---------------------------
# API: Get System Metrics Summary
# ---------------------------
...
def get_system_metrics():
    ...
    try:
        ...
        # Avg waiting time (FCFS)
        cursor.execute("SELECT AVG(waiting_time) as avg_wt FROM process_data WHERE waiting_time IS NOT NULL")
        avg_wt = cursor.fetchone()['avg_wt']
        
        # Avg ML waiting time
        cursor.execute("SELECT AVG(ml_waiting_time) as avg_ml_wt FROM process_data WHERE ml_waiting_time IS NOT NULL")
        avg_ml_wt = cursor.fetchone()['avg_ml_wt']
        
        cursor.close()
        conn.close()
        
        return jsonify({
            "total_processes": total_count or 0,
            "avg_waiting_time": round(float(avg_wt or 0), 2),
            "avg_ml_waiting_time": round(float(avg_ml_wt or 0), 2)
        })
    except Exception as e:
        if conn: conn.close()
        return jsonify({"error": str(e)}), 500
